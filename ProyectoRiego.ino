...
const char* OUT_TOPIC = "$aws/things/AWSExample/shadow/update"; // publish
...
void loop() {
 ...
 unsigned long now = millis();
  if (!mqttClient.connected()) {
    ...
    delay(1000);
   String message = "{\"state\":{\"desired\":{\"temperature\":" + String(temperature) + "," + "\"humidity\":" + String(lecturaHumidity) + "," + "\"modoriego\":\""+estadoRiego+"\"" + "}}}";
   if (isnan(temperature)  ) {
      message = "{\"state\":{\"desired\":{\"temperature\":" + String(0) + "," + "\"humidity\":" + String(lecturaHumidity) + "," + "\"modoriego\":\""+estadoRiego+"\"" + "}}}";
   }
